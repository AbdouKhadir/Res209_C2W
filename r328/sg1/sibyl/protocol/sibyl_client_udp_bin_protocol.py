...
class SibylClientUdpBinProtocol(DatagramProtocol):
    ...
    def sendRequest(self, line):
        ...
        """
        date=int(time.time())
        #taille=4+2+len(line) #taille en octet
        #buf=struct.pack('IH{0}s'.format(len(line.encode('utf-8'))),date,taille,line.encode('utf-8'))
        self.transport.write(building(date,line),(self.serverAddress, self.serverPort))
        pass
    ...
    def datagramReceived(self, datagram, host):
        ...
        #response=struct.unpack_from('{0}s'.format(header[1]-6),datagram,6) #décodage pour récupérer la question
        #ttt=response[0].decode('utf-8').split(':')
        #display=ttt[1].split('\r\n')
        self.clientProxy.responseReceived(decoding(datagram)[2].decode('utf-8'))
