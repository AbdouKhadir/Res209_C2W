...
class SibylServerTimerUdpBinProtocol(DatagramProtocol):
    ...
    def sending_message(self, answer, host_port):
            self.transport.write(answer, (host_port[0],host_port[1]))
            
    def datagramReceived(self, datagram, host_port):
        """Called by Twisted whenever a datagram is received network/r328/sg2/sibyl/protocol/
        ...
        """
        timestamp, length = struct.unpack_from('IH',datagram,0)
        line = struct.unpack_from('{0}s'.format(length-6),datagram,6)
        

        print('message received', line)
        log_length=math.ceil(math.log(length))
        
        answer=self.sibylServerProxy.generateResponse(line)
        
        
       
        reactor.callLater(log_length, self.sending_message , answer, host_port)
        reactor.run()
        pass
